...
/** The always-on vocabulary of trivalibs: JS interop containers and numeric
  ...
  * prefix for a wildcard export"). Top-level definitions of a package do live in
  * a synthetic object named after their source file (`js.scala` →
  ...
  * then be exported twice and clash, so it is hidden from the wildcard
  * (`Arr as _`) and taken from the named export, which carries both the type and
  * the object.
  *
  * ==Maintenance==
  *
...
object core:
  ...
  export trivalibs.utils.js.`js$package`.{Maybe as _, Dict as _, Arr as _, *, given}
