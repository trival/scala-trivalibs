...
trait Vec2ImmutableOps[Vec]:
  ...
  extension (v: Vec)(using Vec2Base[Vec])
    ...
    def inverseSqrt: Vec = create(v.x.inverseSqrt, v.y.inverseSqrt)
    def trunc: Vec = create(v.x.trunc, v.y.trunc)
    def exp2: Vec = create(v.x.exp2, v.y.exp2)
    @scala.annotation.targetName("powVec")
    def pow(e: Vec): Vec = create(v.x.pow(e.x), v.y.pow(e.y))
    @scala.annotation.targetName("powScalar")
    def pow(e: Double): Vec = create(v.x.pow(e), v.y.pow(e))
    ...
    def clamp01: Vec = create(v.x.clamp01, v.y.clamp01)
    ...
    def smoothstep(edge0: Double, edge1: Double): Vec =
      create(v.x.smoothstep(edge0, edge1), v.y.smoothstep(edge0, edge1))
